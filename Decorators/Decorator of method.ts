...
// Этот объект описывает изменение декорируемого метода. Применяется при компиляции 
// в ES5 и выше, при ES3 имеет значение undefined.
...
let tom_m = new UserM("Tom");
tom_m.print = function(){console.log("print has been changed");}
tom_m.print();  // Tom

// Декоратор readonly с помощью выражения descriptor.writable = false; 
// устанавливает, что метод, к которому применяется данный декоратор, не может быть изменен.
...
descriptor.value = function(...args){ }
// Параметр ...args - это все те параметры, которые будут передаваться в функцию. И мы можем логгировать 
// эти параметры. Далее выывается оригинальная функция, которой передаются параметры args:

let returnValue = originalMethod.apply(this, args);
// И таким образом мы можем получить результат вызова оригинальной функции и возвратить его из функции. То 
// есть фактически получается, что мы берем оригинальную функцию, обертываем ее в какую-ту другую функцию, 
...
// Где первый параметр представляет конструктор класса, если метод статический, либо прототип класса, если метод нестатический. 
// А второй параметр представляет имя параметра. И третий параметр представляет порядковый индекс параметра в списке параметров.
...
let tom_m1 = new UserM1("Tom");
tom_m1.setName("Bob");
tom_m1.setName("Sam");
// Декоратор logParameter добавляет в прототип класса новое свойство metadataKey. Это свойство представляет массив, 
// который содержит индексы декорированных параметров.

// Для чтения метаданных из свойства metadataKey применяется декоратор метода logMethod, который перебирает 
// все параметры метода, находит значения параметров по индексам, которые определены декоратором параметроа, 
// и выводит на консоль названия и значения декорированных параметров.
